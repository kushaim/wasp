...
signalProcessGroupIfAlive signal processGroupPid = do
  ...
  when isAlive $ ignoreMissingProcessGroup $ Signals.signalProcessGroup signal processGroupPid
...
ignoreMissingProcessGroup :: IO () -> IO ()
ignoreMissingProcessGroup action =
  action `catchIOError` \ioErr ->
    unless (isDoesNotExistError ioErr) $ ioError ioErr
#endif
